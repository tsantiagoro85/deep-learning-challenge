...
While the present analysis applied neural networks to predict if applicants will be successful if funded by Alphabet Soup, other methods exist that could achieve similar results. For instance, supervised learning, including a logistic regression analysis could also be used. As performed in the previous module and challenge,the data could be split into training and testing sets using train_test_split. A logistic regression model could then be fit using the training data using model.fit(), and predictions could be performed using the testing data using model.predict(). A logistic regression can then be created using the original data and the function LogisticRegression() from sklearn. The model's performance can then be evaluated using a confusion matrix and a classification report. It would be interesting, therefore, to compare the results from this approach with the neural network analysis.
